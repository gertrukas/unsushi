...
export class PermissionListingComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  permissionModel: IPermissionModel = { _id: '', name: '', code: '' };
      ...
      columns: [
        {
          ...
          render: (data: any, type: any, full: IPermissionModel) => `<a href="javascript:;" data-action="view" data-id="${full._id}" class="text-gray-800 text-hover-primary mb-1">${data}</a>`
        },
        {
        ...
        {
          title: 'Creado', data: 'created_at', render: function (data) {
  ...
  create() {
    this.permissionModel = { _id: '', name: '', code: '' };
  }
  ...
  onSubmit(event: Event, myForm: NgForm) {
    ...
    const successAlert: SweetAlertOptions = {
      icon: 'success',
      title: 'Success!',
      text: this.permissionModel._id != '' ? '¡Permiso actualizado correctamente!' : '¡Permiso creado correctamente!',
    };
    ...
    const updateFn = () => {
      this.apiService.updatePermission(this.permissionModel._id, this.permissionModel).subscribe({
        next: () => {
